...
<div className=' flex justify-center relative '>
  <div onClick={closeSide} className={toggle ? 'bg-black opacity-30 fixed top-0 right-0  h-full w-full ' : 'bg-black opacity-30 absolute top-0  -left-full h-full w-full '}  ></div>
  <span onClick={handleToggle} className='cursor-pointer opacity-95 border-2  border-primary bg-primary shadow-xl rounded-full px-1 text-white fixed left-4 top-5 hover:scale-105 hover:bg-yellow-500 transition-all' >{ <IoArrowRedoSharp style={{color: 'yellow', fontSize: '24px' }} />}</span>
  ...
  <div className=' xl:w-10/12 w-full p-6  '>
    <Outlet/>
  </div>
</div>
